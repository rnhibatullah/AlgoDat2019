...
struct management{
    ...
    pthread_mutex_t fin, count_m; 

    //signalize if order has reach its end
    pthread_cond_t o_end;

    //count the finished orders and the number of order
    int fin_o_count, o_count;
...
management* management_create(char* filename, unsigned int num_worker_threads, forwarding_agent* fa){
    ...
    pthread_mutex_init(&new_management->fin,NULL);
    pthread_mutex_init(&new_management->count_m,NULL);
    pthread_cond_init(&new_management->o_end,NULL);

    //number of finished orders at the beginning is zero
    new_management->fin_o_count = 0;
    new_management->o_count = -1;
...
void* get_and_deposit_new_orders(void* man){
    ...
    //have to wait for o_end signal
    pthread_mutex_lock(&m->count_m);   
    while (count_orders != m->fin_o_count) { 
        pthread_cond_wait(&m->o_end,&m->count_m);
    }
    pthread_mutex_unlock(&m->count_m);
...
void deposit_finished_orders(management* m, order* o){
    ...
    while(i< MAX_ORDERS){
        if(m->orders_finished[i] == NULL){
            ...
            pthread_mutex_lock(&m->count_m);
            m->fin_o_count++;
            //signalize that get function can now work
            if (m->fin_o_count == m->o_count) {
                pthread_cond_signal(&m->o_end);
            }
            pthread_mutex_unlock(&m->count_m);
            pthread_mutex_unlock(&m->fin);
            return;
        }
        ++i;
    }
...
unsigned int management_destroy(management* m){  
    //TODO
        ...
        //just destroy all of the mutex and signal
        pthread_mutex_destroy(&m->fin);
        pthread_mutex_destroy(&m->count_m);
        pthread_cond_destroy(&m->o_end);
        free(m);
